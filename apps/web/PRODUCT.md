...
The app runs on an Omarchy Linux workstation, reads local Claude, Codex, Cline, Antigravity, Command Code, and OpenCode transcript/session stores plus Cursor, Hermes, Pi, and usage-collector metric records, and is reached remotely through Cloudflare Tunnel and Access. Nightly analysis runs at 03:15 local time; refreshes use the fixed `omarchy-agent-usage-update --force` command.
...
- The limits portal is a separate, stricter trust tier served from the browser-facing Worker host: it sits behind one Access application (page, assets, and API share a session), and the portal endpoints additionally require the admin audience and reject service tokens by claim; the API origin answers only API routes and never serves the web app; the portal fails closed when unconfigured.
