...
- The limits portal is a separate, stricter trust tier: it is gated by a dedicated path-scoped Access application covering both the page and its API on the tunnel hostname, the Worker host redirects `/limits*` there, service tokens are rejected by claim, and it fails closed when unconfigured.
