...
The `/limits` page and its `/limits/api/*` endpoints are admin-only, and they live on the tunnel hostname: the Worker redirects `/limits*` on the browser-facing host to `$API_ORIGIN/limits`, because service identities can never open the portal. The provisioning script creates one extra path-scoped Access application — `$DASHBOARD_HOSTNAME/limits` (the tunnel hostname) — covering both the page and its API so a single Access session serves both, and writes its audience to `CLOUDFLARE_ACCESS_ADMIN_AUD` (comma-separated) in `dashboard.env`. Every request carrying that audience is verified against the team JWKS with the same issuer/email checks as the main app; requests without a valid token get `401`, tokens for a non-admin audience get `401`, and service tokens (identified by their `common_name` claim) get `403` on any host. When `CLOUDFLARE_ACCESS_ADMIN_AUD` is unset the routes fail closed, including on loopback. Separate Access applications per path cannot share browser sessions, so the page and its API must stay under one application.
